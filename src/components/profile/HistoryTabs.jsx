...
export default function HistoryTabs({
  ...
  return (
    <div className={`card ${styles.card}`}>
      {/* هدر تب‌ها */}
      <div className={`card-header d-flex justify-content-start ${styles.cardHeader}`}>
        <button
        ...
        <button
          className={`${styles.tabBtn} ${activeTab === 'portfolios' ? styles.active : ''}`}
      ...
      <div className={`card-body ${styles.cardBody}`}>
        {/* معاملات */}
        <div className={`${styles.tabContent} ${activeTab === 'transactions' ? styles.show : ''}`}>
            ...
            {transactionsPage.object_list.length > 0 ? (
              <>
                <table className={`table table-bordered ${styles.table}`}>
                  ...
                  <tbody>
                    {transactionsPage.object_list.map(tx => (
                      tx.units > 0 && (
                        <tr key={tx.id}>
                          ...
                          <td>
                            {tx.end_time
                              ...
                              : <span className="text-warning">هنوز بسته نشده</span>}
                          </td>
                        </tr>
                      )
                    ))}
                ...
                <div className={styles.pagerWrap}>
                  <span className={styles.pageInfo}>
                    صفحه {formatNumberFa(transactionsPage.number)} از {formatNumberFa(transactionsPage.paginator.num_pages)}
                  </span>
                  <ul className={styles.pagination}>
                    <li>
                      <button
                        onClick={() => gotoTransactionsPage(transactionsPage.number - 1)}
          ...
          <div className={styles.mobileOnly}>
            {transactionsPage.object_list.length > 0 ? (
              <>
                {transactionsPage.object_list.map(tx => tx.units > 0 && (
                  <div key={tx.id} className={`card ${styles.mobileCard}`}>
                    <div className="card-body">
                      ...
                      <p className={styles.cardTextRow}>
                        ...
                        <span>{formatTehranDateTimeJalali(tx.created_at)}</span>
                      </p>
                      <p className={styles.cardTextRow}>
        ...
        {/* پرتفوها */}
        <div className={`${styles.tabContent} ${activeTab === 'portfolios' ? styles.show : ''}`}>
          <div className={styles.desktopOnly}>
            {portfoliosPage.object_list.length > 0 ? (
              <>
                  ...
                  <tbody>
                    {portfoliosPage.object_list.map(item => (
                      <tr key={item.portfolio.id}>
                        <td>{item.portfolio.portfolio_type === 'isolated' ? 'معمولی' : 'پیشرفته'}</td>
                        <td>{formatNumberFa(item.portfolio.initial_balance)} تومان</td>
                        <td>{formatNumberFa(item.portfolio.total_balance)} تومان</td>
                        <td className={item.profit_loss < 0 ? 'text-danger' : 'text-success'}>
                ...
                <div className={styles.pagerWrap}>
                  <span className={styles.pageInfo}>
                    صفحه {formatNumberFa(portfoliosPage.number)} از {formatNumberFa(portfoliosPage.paginator.num_pages)}
                  </span>
                  <ul className={styles.pagination}>
                    <li>
                      <button
                        onClick={() => gotoPortfoliosPage(portfoliosPage.number - 1)}
          ...
          <div className={styles.mobileOnly}>
            {portfoliosPage.object_list.length > 0 ? (
              <>
                {portfoliosPage.object_list.map(item => (
                  <div key={item.portfolio.id} className={`card ${styles.mobileCard}`}>
                    <div className="card-body">
                      <h5 className="card-title">
                        {item.portfolio.portfolio_type === 'isolated' ? 'معمولی' : 'پیشرفته'}
                      </h5>
                      <p className={styles.cardTextRow}>
                        <span>مبلغ ورودی:</span>
                        <span>{formatNumberFa(item.portfolio.initial_balance)} تومان</span>
                      </p>
                      <p className={styles.cardTextRow}>
                        <span>مبلغ خروجی:</span>
                        <span>{formatNumberFa(item.portfolio.total_balance)} تومان</span>
                      </p>
                      <p className={styles.cardTextRow}>
                        <span>سود/ضرر:</span>
                        <span className={item.profit_loss < 0 ? 'text-danger' : 'text-success'}>
                          {formatNumberFa(item.profit_loss)} تومان
                        </span>
                      </p>
                      <p className={styles.cardTextRow}>
